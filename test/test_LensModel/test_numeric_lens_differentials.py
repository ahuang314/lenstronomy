...
class TestNumerics(object):
    ...
    def test_flexion(self):
        ...
        f_xxx, f_xxy, f_xyy, f_yyy = lensModel.flexion(
            x=1.0, y=1.0, kwargs=kwargs, diff=0.0001
        )
        ...
        f_xxx, f_xxy, f_xyy, f_yyy = lensModel.flexion(
            x=1.0, y=1.0, kwargs=kwargs, diff=0.0001, hessian_diff=0.001
        )
        npt.assert_almost_equal(f_xxx, g1, decimal=2)
        npt.assert_almost_equal(f_xxy, g2, decimal=2)
        npt.assert_almost_equal(f_xyy, g3, decimal=2)
...
class TestNumericsProfile(object):
    ...
    def test_ctnfw_gauss_dec(self):
        kwargs = {"rho_s": 5, "r_s": 5.0, "r_trunc": 10.0, "r_core": 0.3, "a": 2}
    ...
    def test_sersic_ellipse_gauss_dec(self):
        kwargs = {"n_sersic": 1.0, "R_sersic": 2.0, "k_eff": 1.0, "e1": 0.04, "e2": 0.0}
    ...
    def test_sis_truncate(self):
        kwargs = {"theta_E": 0.5, "r_trunc": 2.0}
    ...
    def test_PJaffe(self):
        kwargs = {"sigma0": 1.0, "Ra": 0.2, "Rs": 2.0}
        lens_model = ["PJAFFE"]
        self.assert_differentials(lens_model, kwargs)
    ...
    def test_NIE(self):
        kwargs = {"theta_E": 2.0, "e1": 0.1, "e2": 0.0, "s_scale": 0.04}
        lens_model = ["NIE"]
        self.assert_differentials(lens_model, kwargs)

    def test_NIE_simple(self):
        kwargs = {"b": 2.0, "q": 0.3, "s": 0.04}
    ...
    def test_EPL_Q_PHI(self):
        kwargs = {"theta_E": 2.0, "q": 0.9, "phi": 0.1, "gamma": 2.13}
    ...
    def test_cnfw(self):
        kwargs = {"Rs": 5.5, "alpha_Rs": 1.0, "r_core": 0.5}
        lens_model = ["CNFW"]
    ...
    def test_cse(self):
        kwargs = {"a": 2, "s": 1.0, "e1": 0, "e2": 0, "center_x": 0.0, "center_y": 0.0}
        lens_model = ["CSE"]
        self.assert_differentials(lens_model, kwargs, potential=True)
